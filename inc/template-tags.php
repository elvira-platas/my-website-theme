...
if ( ! function_exists( 'kilka_posted_on' ) ) :
	...
	function kilka_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'On %s', 'post date', 'kilka' ),
...
if ( ! function_exists( 'kilka_posted_by' ) ) :
	...
	function kilka_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'By %s', 'post author', 'kilka' ),
...
if ( ! function_exists( 'kilka_entry_footer' ) ) :
	...
	function kilka_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'kilka' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Category %1$s', 'kilka' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
